#!/usr/bin/env node
// Like test-onboard.mjs, but also waits for the post-activation bootstrap
// step (follows/feeds/blocklists from bootstrap-config.js) to finish, so we
// can see its summary/failures before the browser closes.
